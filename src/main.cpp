...
void turnChessType()
{
	...
	if (nowCDTime == &blackChessCDTime)
		nowCDTime = &whiteChessCDTime;
	else
		nowCDTime = &blackChessCDTime;
}
...
void showWinner()
{
	...
	{
		MessageBox(GetHWnd(), _T("白棋玩家胜利(^_^)Ohohoho....."), _T("胜负情况"), MB_OK);
		whiteScore++; // 白棋得一分
	}
	else if (gameResult == 2)
	{
		MessageBox(GetHWnd(), _T("黑棋玩家胜利(^_^)Hiahiahia..."), _T("胜负情况"), MB_OK);
		blackScore++; // 黑棋得一分
	}
}
...
int chessSum(int row, int col, int dx, int dy)
{
	...
	while (true)
	{
		row += dx;
		col += dy;
		if (isOverBorder(row, col))
			return sum;
		if (gameBoard[row][col] == nowChessType)
			sum++;
		else
			return sum;
	}
